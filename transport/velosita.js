velosita = () => {
  console.log("velosita");
  console.log(cek("-tiket-pesawat-"));

  DIVKonf = document.createElement("div");
  DIVKonf.id = "DIVKonf";
  DIVKonf.innerHTML =
    "<a id='KopasAplikasi'>Aplikasi</a><a id='KopasKonf'>Konfirmasi</a><textarea id='TextKonf'></textarea>";
  document.body.appendChild(DIVKonf);
  StyleKonf = document.createElement("style");
  StyleKonf.innerHTML =
    "#DIVKonf{position:fixed;top:40%;right:-100px;z-index:99999999;background:#53B9EC;padding:5px 10px;border-radius:5px;box-shadow:-1px 1px 3px #000;transition: all 0.5s ease;}" +
  ...
  //========= Pencarian Tiket Pesawat
    ...
    qs("#KopasKonf").onclick = function () {
      ...
      var d = new Date(
        parseInt(Thn.substr(0 - 4)),
        parseInt(Thn.substr(-6, 2)) - 1,
        parseInt(Thn.substr(-8, 2)) - 1,
      );
      ...
      [].forEach.call(OW, function (A) {
        ...
        dt.forEach(function (z, i) {
          if (i && i < dt.length - 2)
            Teks += z.replace(" - ", "```)-").replace(/\s/g, "(```") + "```)\n";
          else if (i == dt.length - 2)
            Teks += "```" + z.split(" (")[0] + "```\n";
        });
        Teks += "\n";
  ...
  //========= Konfirmasi Tiket Pesawat
    ...
    qs("#KopasKonf").onclick = function () {
      //===== Rute
      AllTblPenerbangan = Contains("h3", "penerbangan pergi").parentElement
        .nextElementSibling;
      ...
      let firstLength = false;
      [].forEach.call(TblPenerbangan, function (A, idx) {
        let TD = A.qsa(":scope>td");
        if (firstLength === false) firstLength = TD.length;

        let min_1 = TD.length < firstLength ? -1 : 0;
        Teks += "*Kode: " + TD[idxKode].innerText.trim() + "*\n";
        Teks +=
          TD[idxMaskapai].qs("img").Attr("alt").toTitleCase +
          "(" +
          TD[0].qsa("div")[1].innerText.trim() +
          ")" +
          "\n";
        Teks +=
          TD[idxBrgkt + min_1].innerText
            .trim()
            .replace(/\s/g, "")
            .replace(/,/g, ", ") + "\n";
        let TD3Kode = TD[idxDetail + min_1].qsa("strong");
        let TD3Jam = TD[idxDetail + min_1].qsa("span");
        // let TD3Kode = TD[idxDetail].qsa("strong");
        // let TD3Jam = TD[idxDetail].qsa("span");
        // console.log(TD[idxKode].qsa("strong"));
        // console.log(TD);

        Teks +=
          " - " +
          TD3Kode[0].innerText.trim() +
          " (" +
          TD3Jam[0].innerText.trim().toUpperCase() +
          ")\n";
        Teks +=
          " - " +
          TD3Kode[2].innerText.trim() +
          " (" +
          TD3Jam[2].innerText.trim().toUpperCase() +
          ")\n"; /**/
        console.log(idxBagasi);
        console.log(TD[idxBagasi + min_1]);

        if (idxBagasi)
          Teks +=
            "Bagasi:" + TD[idxBagasi + min_1].qs("strong")?.innerText + "\n\n";
      });
      ...
      Penumpang = Contains(
        "h3",
        "Data Penumpang",
      ).parentElement.nextElementSibling.qsa("table.table>tbody>tr");
      Teks += "Penumpang:\n";
      [].forEach.call(Penumpang, function (A, B) {
        Teks +=
          ...
          1 +
          ". " +
          A.qs(".text-primary").innerText.replace("/ ", "") +
          "\n";
      });
      Teks +=
        "\nHarga: Rp. " +
      ...
      if (cek("-booked")) {
        Teks +=
          "Limit: " +
          Contains("li", "Batas Pembayaran")
            .qs("text")
            .innerText.trim()
            .replace(/(-20|-|,)/g, "")
            .replace(/\s/g, ",").toTitleCase +
          "\n";
      }
    ...
    qs("#KopasAplikasi").onclick = function () {
      ...
      [].forEach.call(TblPenerbangan, function (A) {
        ...
        let Mask = TD[idxMaskapai].qs("img").Attr("alt").toTitleCase;
        if (Mask == "Transnusa") noTd = noTd * -1;
        Mask = "Vel-" + Mask;
        Teks.maskapai = "maskapai" in Teks ? Teks.maskapai + "|>" + Mask : Mask;
        var flight = TD[idxMaskapai].qsa("div")[1].innerText.trim();
        Teks.flight = "flight" in Teks ? Teks.flight + "|>" + flight : flight;
        let rute = TD[idxDetail + min_1].qsa("strong");

        rute = rute[0].innerText.trim() + "-" + rute[2].innerText.trim();
        Teks.rute = "rute" in Teks ? Teks.rute + "|>" + rute : rute;
        // if (noTd == 0) {
        var kode = TD[idxKode].innerText.trim();
        Teks.kode = "kode" in Teks ? Teks.kode + "|>" + kode : kode;
        // }
        var DepArr = TD[idxBrgkt + min_1].innerText
          .split("\n")[1]
          .trim()
          .split(" ");
        ...
        var Tg = (Tg < 10 ? "0" : "") + Tg;
        var Jam = TD[idxDetail + min_1].qsa("span");
        var dep =
          DepArr[2] +
          Bln +
          Tg +
          Jam[0].innerText.trim().replace(":", "").split(" ")[0];
        var arr =
          DepArr[2] +
          Bln +
          Tg +
          Jam[2].innerText.trim().replace(":", "").split(" ")[0];
      ...
      //==== Penumpang
      [].forEach.call(
        Contains("h3", "Data Penumpang").parentElement.nextElementSibling.qsa(
          "table.table>tbody>tr",
        ),
        function (A, B) {
          var penumpang = A.qs(".text-primary").innerText.replace("/ ", "");
          Teks.penumpang =
            "penumpang" in Teks ? Teks.penumpang + "|>" + penumpang : penumpang;
        },
      );
      //==== Status
      Teks.status = cek("-issued") ? "Issued" : "Booking";
      //==== Tgl Status
      var TglSts = Contains("li", "Tanggal Pemesanan")
        .qs("text>strong")
        .innerText.split("-"); //Created Date
      var d = new Date();
      var j = d.getHours();
      var j = (j < 10 ? "0" : "") + j;
      var m = d.getMinutes();
      ...
      //==== NTA
      Teks.nta = Contains("td", "Harga NTA")
        .nextElementSibling.innerText.trim()
        .replace(/(Rp|\s|\.|,|-)/g, "");
      //==== Profit
      Teks.profit = Contains("td", "Komisi")
        .nextElementSibling.innerText.trim()
        .replace(/(Rp|\s|\.|,|-)/g, "");
      //==== Harga
      Teks.harga = Contains("td", "Harga Total")
        .nextElementSibling.innerText.trim()
        .replace(/(Rp|\s|\.|,|-)/g, "");

      // Teks.nta = Teks.harga;
      Teks.harga = (parseInt(Teks.nta) / 0.97).toFixed(0);
      Teks.profit = Teks.harga - parseInt(Teks.nta);
  ...
  //========= Pencarian Tiket Kereta
    ...
    qs("#KopasKonf").onclick = function () {
      ...
      OW.forEach(function (A) {
        var Data =
          A.nextElementSibling.nextElementSibling.qsa("div.search-row>div");
        if (Data.length > 1) {
          ...
          Teks +=
            TD[1].qs("small").innerText.trim().substr(-4, 3) +
            "(```" +
            TD[1].qs("h4").innerText.trim() +
            "```)-";
          Teks +=
            TD[2].qs("small").innerText.trim().substr(-4, 3) +
            "(```" +
            TD[2].qs("h4").innerText.trim() +
            "```)\n";
          for (var z = 0; z < Data.length / 2; z++) {
            var i = z * 2 + 1;
            var TD = Data[i].qsa("td");
            Teks +=
              TD[0].qs("td").innerText.trim().substr(0, 3) +
              "(```" +
              TD[0].qs("small>b").innerText.trim() +
              "```)" +
              ": " +
  ...
  //========= Konfirmasi Tiket Kereta
    ...
    qs("#KopasKonf").onclick = function () {
      ...
      Detail.forEach(function (A) {
        ...
        Teks +=
          " - " +
          TD[3].qsa("strong")[0].innerText.trim() +
          "(```" +
          TD[3].qsa("span")[0].innerText.trim() +
          "```)\n";
        Teks +=
          " - " +
          TD[3].qsa("strong")[1].innerText.trim() +
          "(```" +
          TD[3].qsa("span")[1].innerText.trim() +
          "```)\n\n";
      });
      ...
      Teks +=
        "\nHarga: ```" +
        Contains("td", "Harga Tiket").nextElementSibling.innerText.trim() +
        "```\n";
      Teks += "Status: " + (cek("-issued") ? "Issued" : "Booking") + "\n";
      if (cek("-booked")) {
        Teks +=
    ...
    qs("#KopasAplikasi").onclick = function () {
      ...
      Detail.forEach(function (A) {
        ...
        let arr =
          ...
          Bln +
          Tg +
          Jam[1].innerText.trim().replace(":", "").split(" ")[0];
        Teks.dep = "dep" in Teks ? Teks.dep + "|>" + dep : dep;
        Teks.arr = "arr" in Teks ? Teks.arr + "|>" + arr : arr;
      });
      ...
      Teks.profit = Contains("td", "Komisi")
        ...
        .replace(/(Rp|\s|\.|,|-)/g, "");
      //==== Harga
      Teks.harga = Contains("td", "Harga Tiket")
        .nextElementSibling.innerText.trim()
        .replace(/(Rp|\s|\.|,|-)/g, "");
