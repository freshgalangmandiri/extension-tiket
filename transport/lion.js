lion = () => {
  var imgSrc = "none";
  if ($("#imgStepHeader").length > 0) {
    var imgSrc = $("#imgStepHeader").attr("src");
  }
  ...
  if (imgSrc.indexOf("step1") > -1) {
    ...
    if (cek("confirmation") || cek("ticketbooking")) {
      ...
      $("#ctl00_ContentPlaceHolder1_lblBookingReloc").attr(
        "id",
        "lblRefNumber",
      );
      $("#ctl00_ContentPlaceHolder1_gvBookingsItinerary").attr(
        "id",
        "FlightTable",
      );
      ...
      for (var z = 1; z < trPener.length; z++) {
        ...
        td.eq(0).append(
          "<div class='FlightNumberInTable' style='display:none'>" +
            td.eq(0).html() +
            " " +
            td.eq(1).html() +
            "</div>",
    ...
    //=====tanggal==>
    if ($("#lblIssuedDate") > 0)
      zTanggal = $("#lblIssuedDate").text().tanggal("num");
    ...
    TR.forEach((tr, i) => {
      if (tr.qsa("#FlightTableFooter").length > 0) return;
      let td = tr.qsa("td"),
      ...
      zRute +=
        br +
        td2[0].trim().slice(-4).substring(0, 3) +
        "-" +
        td3[0].trim().slice(-4).substring(0, 3);
    ...
    //===== Limit ==
    if (zStatus == "Booking")
      zLimit = qs("#lblPayByDate").innerText.tanggal("num");
    ...
    zNTA = zNTA; //NTA
    // zNTA = qs("#lblTotalFares").innerText.replace(/(,)/g, ""); //Harga
